...
/// One completed, overlap-normalized long-form window.
///
/// Token frame indices are already translated into the full recording's
/// timeline. Callers can append each progress value directly and persist a
/// crash-safe partial transcript without retaining audio for earlier windows.
public struct TranscriptionProgress: Sendable {
    public let chunkIndex: Int
    public let tokenIds: [Int]
    public let frameIndices: [Int]
    public let durations: [Int]
    public let completedAudioDurationSeconds: Double
    public let totalAudioDurationSeconds: Double

    public var fractionCompleted: Double {
        guard totalAudioDurationSeconds > 0 else { return 0 }
        return min(1, completedAudioDurationSeconds / totalAudioDurationSeconds)
    }
    ...
    public init(
        ...
        totalAudioDurationSeconds: Double
    ) {
        self.chunkIndex = chunkIndex
        self.tokenIds = tokenIds
        self.frameIndices = frameIndices
        self.durations = durations
        self.completedAudioDurationSeconds = completedAudioDurationSeconds
        self.totalAudioDurationSeconds = totalAudioDurationSeconds
    }
}
