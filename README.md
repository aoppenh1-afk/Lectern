...
Dev releases also require the original certificate. GitHub Actions needs two secrets in the `release-signing` environment: `LECTERN_SIGNING_P12_BASE64`, the base64-encoded `.p12` containing the original certificate and private key, and `LECTERN_SIGNING_P12_PASSWORD`, that archive's password. Restrict the environment to the `main` branch. The dev workflow imports it into a temporary keychain, verifies its fingerprint, and removes the keychain after publishing. Missing secrets or a different certificate stop the release rather than publishing an ad-hoc build. Keep these values out of repository files, Actions variables, and logs. Public readers and fork pull requests cannot access the secrets; code running in an authorized release job can, so review changes to release workflows and build scripts before merging.

Ad-hoc builds change their designated requirement on every rebuild, so macOS treats each update as a different app for Screen & System Audio Recording permission. Signing every stable and dev update with the original certificate removes this identity change without a paid Apple Developer account. After switching from an ad-hoc build, grant recording permission once for the signed app and restart Lectern. Verify retention on the next signed update; this does not suppress macOS's own periodic consent prompts or resolve the separate Keychain behavior described below. See [Apple's explanation of TCC and ad-hoc signing](https://developer.apple.com/forums/thread/819406) and [code signing requirements](https://developer.apple.com/documentation/technotes/tn3127-inside-code-signing-requirements).
