...
Dev releases also require the original certificate. GitHub Actions needs two repository secrets: `LECTERN_SIGNING_P12_BASE64`, the base64-encoded original `.p12` backup including its private key, and `LECTERN_SIGNING_P12_PASSWORD`, that backup's password. The dev workflow imports it into a temporary keychain, verifies its fingerprint, and removes the keychain after publishing. Missing secrets or a different certificate stop the release rather than publishing an ad-hoc build.
