...
- Keep short parenthetical glosses and citations when useful. Use child items for independently developed comparisons, preserving each view and qualifier. Keep fractions such as `1/3` in their ordinary numeric order. Preserve a continuous quotation as a quotation instead of splitting it into bullets to work around display direction.
