...
- English carries the connective reasoning and ordinary descriptions. Write ordinary words once in English, such as `house`, `wall`, or `money`; use Hebrew-script Torah terms directly when they name the concept being discussed.

### Translations and source wording

- Avoid redundant bilingual pairs in either direction. Write `house`, not `house (בית)`, when describing an ordinary house. A word's appearance in a Hebrew source does not by itself make its translation worth including.
- Keep Hebrew or Aramaic source wording alongside an English explanation when it anchors the explanation to a passage discussed in the shiur, or when the argument depends on the exact wording. This can be a short parenthetical quotation from the משנה or גמ׳, or even one word if the speaker analyzes that word. Use only wording supported by the supplied source; never reconstruct a quotation by translating the English back into Hebrew.
- Give a short English gloss for a Torah term when the speaker develops its definition or when a rare term needs clarification to follow the argument. Give it at the point of explanation rather than repeating it at every mention. A passing switch between equivalent English and Hebrew words does not require a bilingual pair in the notes.
- Keep parenthetical citations and substantive asides. For translations, ask whether the second language preserves source wording or an explanation needed for the reasoning. If it only repeats ordinary vocabulary, omit the duplicate. Apply this distinction within the requested scope when revising existing notes too.
...
- Keep a Hebrew quotation in one continuous logical reading order across commas, colons, semicolons, ellipses, question marks, and line wraps. Those marks punctuate the quotation; they do not start a new left-to-right component. Store opening quote or parenthesis, the complete quotation, then its closing delimiter. For example: `**גמ׳:** "אי אמרת בשלמא: להחזיר תנן, היינו דשני בין תוכה לעל גבה; אלא אי אמרת: לשהות תנן, מה לי תוכה מה לי על גבה?"`. The bold label remains a separate component to the left; the whole quotation reads RTL.
- A parenthetical citation inside a Hebrew source label belongs to that source's RTL phrase. Keep it inside the same bold label: `**לפי תוס׳ (ורא״ש פרק ד׳):**` and `**פני יהושע (דף ל״ז. בשיטת רש״י)**`. Read the source name first at the right, then the parenthetical to its left. A fully Hebrew heading follows the same rule.
- A parenthetical aside in an English sentence remains a separate component in the sentence's LTR order. For example, in `an איסור דרבנן of אטו שבת (שמא יחזיר בשבת)`, keep the aside to the right of `אטו שבת`, with RTL Hebrew inside the parentheses. A parenthesized Hebrew quotation is also its own component, with both delimiters enclosing the complete RTL quotation even when it wraps. Preserve this distinction when revising; do not move every parenthetical to the same side.
- Use the translations and source wording rules above to decide which parentheticals to keep. Use child items for independently developed comparisons, preserving each view and qualifier. Keep fractions such as `1/3` in their ordinary numeric order. Preserve a continuous quotation as a quotation instead of splitting it into bullets to work around display direction.
...
4. The chosen layout is consistent throughout, and the shiur orthography follows the Hebrew-script and English-spelling rules above, with `״` and `׳` for Hebrew abbreviations. Bilingual pairs pass the translations and source wording check above.
5. Parentheses and quotes are paired in logical order; source punctuation, question marks, and numeric fractions retain their meaning. Mixed-language comparisons remain readable as nested items. The output contains no added direction controls; Hebrew characters and words stay in logical reading order, with amud marks after their numerals. An inline bold source precedes its explanation in the LTR layout, while each Hebrew passage reads RTL across punctuation and line wraps. Source-internal parentheses follow their Hebrew source; English-sentence asides keep their LTR component position.
